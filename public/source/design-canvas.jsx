...
// Resolve the project's small section-wrapper components so deep links can
// focus artboards that live in files loaded before the page's main App.
// These wrappers are intentionally pure functions whose names end in Section.
function dcExpandSections(children) {
  const out = [];
  dcFlatten(children).forEach((c) => {
    const name = c && typeof c.type === 'function' ? (c.type.name || '') : '';
    if (c && c.type !== DCSection && /Section$/.test(name)) {
      try { out.push(...dcExpandSections(c.type(c.props || {}))); }
      catch { out.push(c); }
    } else out.push(c);
  });
  return out;
...
function DesignCanvas({ children, minScale, maxScale, style }) {
  ...
  const sectionOrder = [];
  dcExpandSections(children).forEach((sec) => {
